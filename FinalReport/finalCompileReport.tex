...
Throughout this analysis, the goal is to look beyond industry standard metric of Mispredictions Per Kilo-Instruction (MPKI) reductions. By examining where regressions occur and how misprediction ceilings evolve, this report extracts broader architectural insights into the relationship between history, data, and design complexity. Ultimately, this report serves as both a performance evaluation and an exploratory report into the future of robust and effective branch prediction.

\newpage
\section{Methodology}

This section describes how I obtained all reported values and figures, beginning with the raw
simulator outputs and ending with the plots included in the final report. The goal of this section
was to make the evaluation process transparent and reproducible, and to clearly explain why each
metric was required to analyze branch predictor behavior beyond simple aggregate accuracy.

\subsection{Simulator and Experimental Setup}

All experiments were run using the official Championship Branch Prediction (CBP) 2025 cycle-accurate
simulator (\texttt{cbp}). The simulator models a superscalar out-of-order core using a window-based
pipeline and memory hierarchy, and it exposes branch predictor callbacks at multiple pipeline stages
(fetch, decode, execute/resolve, and commit) via the CBP predictor interface.

To ensure that results are reproducible, all predictors were evaluated under the same simulator
configuration and microarchitectural parameters, on the same trace set, and under the same conditional
branch predictor storage budget. 

The workload suite was the CBP 2025 training trace set: 105 compressed instruction traces spanning six
categories ie Int (37), Fp (14), Web (26), Infra (16), Compress (8), and Media (4). Unless explicitly
stated otherwise, every predictor result in this report reflects running the full trace set under
identical conditions.

While the baseline suite comprises 105 traces, the final number of evaluated traces varies in certain
experiments (ranging from 101 to 105) due to occasional simulator timeouts/crashes that prevent some
traces from producing valid per-trace CSV rows, and due to analysis-specific filtering
applied in selected comparisons.

\subsection{Experimental Data Generation Pipeline}

All experimental results came from cycle-accurate simulator runs on a fixed set of instruction
traces. I evaluated each predictor configuration independently on the same trace set, so any
performance differences I observed were due to predictor design rather than workload variation.

For each predictor, the simulator gave me a per-trace CSV file with an identical schema. Each
row was a single trace and each column was a specific metric. Since the schema was the same
across all predictors, I could line up the CSV files by trace identifier without any manual
data manipulation.

These CSV files were the only source of numbers in this report. I used a scripted analysis
pipeline to process them, which did the following:
\begin{itemize}
    \item Loaded and aligned per-trace CSV files across predictors by trace identifier.
    \item Computed derived metrics (MPKI, IPC, CycWPPKI, etc.) from raw simulator outputs.
    \item Generated pairwise comparison figures and delta scatter plots.
    \item Produced summary tables and composite ranking scores.
\end{itemize}


\subsubsection{Instruction Throughput Metrics}

I recorded the total number of retired instructions (\textbf{Instr}) and the total number of
simulated cycles (\textbf{Cycles}) for each trace. From these values, I computed instructions per
cycle (IPC):

\[
\text{IPC} = \frac{\text{Instr}}{\text{Cycles}}
\]

IPC was used as an end-to-end performance metric, as it reflected not only branch prediction
accuracy but also the downstream cost of mispredictions, including pipeline flushes and recovery
latency.

\subsubsection{Branch Frequency and Misprediction Counts}

To characterize control-flow behavior, the simulator recorded the total number of conditional branches
(\textbf{NumBr}) and the number of mispredicted branches (\textbf{MispBr}) per trace. From these raw
counts, I derived and reported the branch and misprediction densities:

\[
\text{BrPerCyc} = \frac{\text{NumBr}}{\text{Cycles}}, \qquad
\text{MispBrPerCyc} = \frac{\text{MispBr}}{\text{Cycles}}
\]

These metrics captured how frequently branches occurred and how often mispredictions disrupted
execution relative to overall runtime.

These density metrics were utilized strictly for intermediate trace profiling and workload
characterization, rather than for the final composite predictor evaluation.

\subsubsection{Accuracy-Oriented Metrics}

Branch prediction accuracy was evaluated using two complementary metrics. First, I computed the
misprediction rate (MR):

\[
\text{MR} = 100 \cdot \frac{\text{MispBr}}{\text{NumBr}}
\]

While MR measured per-branch accuracy, it did not account for instruction mix or execution length.
For this reason, I relied primarily on mispredictions per kilo-instruction (MPKI):

\[
\text{MPKI} = 1000 \cdot \frac{\text{MispBr}}{\text{Instr}}
\]

MPKI normalized mispredictions by instruction count and provided a stronger correlation with
overall performance impact.

\subsubsection{Wrong-Path Execution Cost Metrics}

Accuracy alone did not fully capture the cost of branch mispredictions. To quantify recovery
overhead, I used the simulator-reported number of cycles spent on wrong-path execution

(\textbf{CycWP}). from this value, I computed wrong-path execution cost normalized by instruction
count:

\[
\text{CycWPPKI} = 1000 \cdot \frac{\text{CycWP}}{\text{Instr}}
\]

These metrics distinguished predictors that reduced misprediction frequency from those that also
reduced the cost of each misprediction.

\subsection{Predictor-versus-Baseline Comparison Figures}

Pairwise comparison figures were generated using a Python and Matplotlib. The python file loads the per-trace CSV files for two predictors and
produces a 2$\times$2 comparison figure (containing 4 plots).

The resulting figures were saved as PNG files and included directly in the LaTeX report.

I chose a 2$\times$2 panel layout because each subplot reveals a different aspect of behavior: the MPKI/IPC
scatter plots expose per-trace tradeoffs and outliers, the MPKI-difference histogram shows whether gains are
broad-based or concentrated, and the normalized mean bars summarize overall direction across complementary
metrics without hiding trace-level structure.

\subsubsection{Scatter Plots}

The top-left subplot showed a scatter plot of MPKI values, with baseline MPKI on the x-axis and
the evaluated predictor MPKI on the y-axis. A diagonal reference line indicated equal
performance; points below the diagonal represented an improvement over the baseline.

The top-right subplot followed the same structure for IPC. In this case, points above the
diagonal indicated higher instruction throughput.

\subsubsection{MPKI Difference Histogram}

The bottom-left subplot visualized the distribution of per-trace MPKI differences:

\[
\text{MPKI}_{\text{pred2}} - \text{MPKI}_{\text{pred1}}
\]

Negative values indicated improved accuracy relative to the baseline and revealed whether
performance gains were widespread or limited to a subset of traces.

\subsubsection{Normalized Mean Metric Comparison}

The bottom-right subplot compared normalized mean values of MPKI, MR, and CycWPPKI. Each metric
was normalized by the larger of the two predictors’ mean values. Because normalization was
performed independently for each comparison, bar heights were intended for within-figure
comparison only and should not be compared across different predictor pairs.

\subsection{Delta-Based Comparison and Subsumption Analysis}

In addition to direct predictor-versus-baseline comparisons, I performed a delta-based analysis
to examine how predictors related to one another on a per-trace basis. This approach emphasized
relative predictor behavior rather than aggregate averages.

\subsubsection{Delta Definition}

For any two predictors $A$ and $B$, I defined the per-trace delta as:

\[
\Delta = \text{MPKI}_A - \text{MPKI}_B
\]

With this definition, a positive delta indicated that predictor $B$ achieved lower MPKI than
predictor $A$ on that trace.

\subsubsection{Trace Filtering}

Some traces were not informative for delta analysis. In particular, traces that reported zero
MPKI for one or both predictors indicated that no mispredictions occurred during execution. To
avoid distorting delta distributions with such cases, I excluded any trace where either predictor
had zero MPKI.

To avoid distorting visualizations, this filtering rule was applied strictly to the delta scatter
plots. Zero-MPKI traces were retained in the tabular summaries to accurately reflect absolute
performance gains.

\subsubsection{Delta Visualizations}
I analyzed MPKI deltas using scatter plots showing per-trace delta values, 
with traces ordered along the x-axis by baseline MPKI, from most difficult 
to easiest. This ordering made it easier to observe whether a predictor's 
advantage was concentrated in challenging workloads or distributed across 
the benchmark set.


\subsection{Improvement Classification and Success Ratio}
\label{sec:improvement-classification}

To classify per-trace outcomes, I introduced an error MPKI of of $\pm 0.1$ MPKI to account
for small numerical variations in simulator outputs. I selected $\pm 0.1$ MPKI as a practical
significance rate (roughly one misprediction per 10,000 instructions), so extremely small deltas
are not over-interpreted as meaningful improvements. A trace was classified as \emph{improved} if
the evaluated predictor reduced MPKI by more than 0.1 relative to the comparison baseline. Traces
with MPKI differences within $\pm 0.1$ were treated as equivalent.

Using this classification, the \emph{Improve/Total} value reported in tables reflected the number
of improved traces divided by the total number of evaluated traces. The \emph{Success Ratio (\%)}
was computed as the percentage of traces classified as improved under this criterion.

\subsection{Composite Predictor Ranking}

To provide an overall comparison across multiple metrics, I constructed a composite ranking
scheme. Predictors were ranked independently across five core performance metrics: MPKI,
misprediction rate (MR), IPC, total simulated cycles (Cycles), and CycWPPKI. These per-metric
ranks were then summed to produce a composite score, where lower values indicated stronger
overall performance. This approach ensured that no single metric dominated the final ranking.

\subsection{Rationale for Metric Selection}

Each metric used in this report captured a distinct aspect of branch predictor behavior. MPKI
quantified prediction accuracy, IPC reflected overall performance impact, CycWPPKI exposed
recovery cost, and delta-based comparisons revealed relative predictor dominance. Together, these
metrics enabled conclusions that were robust and not overly dependent on any single measurement.

I used MPKI rather than MR as the primary accuracy metric because MPKI normalizes by instruction count and
therefore better reflects end-to-end performance impact across traces with very different branch densities.

While IPC normalizes instruction throughput, raw simulated Cycles were included in the composite
evaluation to capture the absolute reduction in execution time across the fixed benchmark suite.

I also used CycWPPKI rather than raw CycWP because normalizing wrong-path cycles by instruction count makes recovery
overhead comparable across traces of different lengths and instruction mixes.
% ==========================================
% SECTION 1: LVCP
% ==========================================
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{load_value_vs_baseline_plots.png}
...
This predictor includes several notable design techniques that were useful to study.
...
The normalized-mean subplot shows a substantial gain in MPKI. In terms of raw numbers, the MPKI for the register value aware predictor is 13.28\% better than the baseline. IPC improves modestly, while both MPKI and misprediction rate decrease. This pattern suggests that the gains are concentrated in hard-to-predict branches where history-based correlation is weak. Since the predictor builds on top of TAGE and selectively injects register information when it is useful, it can capture value-dependent behavior while retaining strong baseline behavior.
...
Approximately 15/105 benchmarks reported ``similar'' MPKIs, while the remainder show clear gains in MPKI.

It is not very surprising that a lot of the improvements in the MPKI (where the difference is more than 1) belong to the INT benchmark. Now that intuitively makes sense since most of the INT benchmarks are used to do integer computations, array indexing, control flow decisions, etc. But another thing to consider is that the success rate (i.e., \#improved/total) is 75\%, which means that INT benchmarks have a wide variety. I assume it does well when it involves branches that use register values, and it turns out that the MPKI speedups are very noisy, either having significant improvements or having little to no improvement, mostly leaning towards the improvement side. RVA-Toru performs well on INT because it exploits value.
...
MispBrPerCyc   & 0.0100      & 0.0089      & $\downarrow$ 10.23\% better \\
...
The structural correlator was also revised. Many of these optimizations are derived from the 2024 TAGE engineering report \textit{[André Seznec. 2024. TAGE: an engineering cookbook. Technical Report 9561]}. 

Something that I found notable was the use of a tagged IMLI (Inner Most Loop Iterator) to address fall-through mispredictions on the last loop iteration. Tagging is important because different loops would otherwise interfere and overwrite each other's counters. \textbf{A key design detail is that the predictor uses two IMLI tables with different roles.} The second component, the branch-context IMLI, targets branches without fixed loop iterations by using histories based on the previous anchor branch. Anchor branches can introduce bias; however, the BrIMLI correlator tends to produce a conservative bias that is still counting-based rather than purely pattern-based.
...
For per-trace improvement/equivalence classification, I use the tolerance threshold defined in Section~\ref{sec:improvement-classification}. This keeps the trace-level comparisons consistent across all predictors and avoids over-interpreting very small numerical differences.
...
The results show that the TAGE-SC 2025 predictor achieves a consistently lower MPKI than the baseline implementation, with an overall improvement of approximately 11\%. This is a meaningful gain, especially given that both predictors operate under similar storage and structural constraints. Across nearly all workloads, the 2025 design outperforms the baseline, confirming the effectiveness of its architectural refinements.

A notable exception appears in a small subset of the infrastructure benchmarks. Of the few cases where the baseline predictor performs better, three out of four occur within the \textit{infra} category. While these differences are minor in magnitude, they suggest a systematic pattern rather than random noise.
...
\midrule
AAverage    & 61/105 & 58.1\% & 70/105 & 66.7\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Hybrid Robustness:} RVA-Toru sits atop a TAGE-like base. It defaults to TAGE when register correlation is weak. The lack of significant regressions (points above the line in Figure~2 (RVA vs. Baseline MPKI scatter)) confirms that RVA is a safe, additive optimization.
\end{itemize}
...
Overall, this ranking table reinforces the central conclusion of this report: while history-based predictors have largely converged in performance, incorporating data-flow information, especially register values, provides the most consistent and system-wide gains.
...
This report of the CBP 2025 predictors highlights a clear shift in how modern branch prediction is improving. By comparing the Load Value Correlator (LVCP), RVA-Toru, and several TAGE-based predictors, a consistent pattern emerges: further gains from control-flow history alone are becoming harder to achieve, while data-driven techniques are providing the most meaningful improvements.
...
\noindent I used AI tools in a limited way to support writing and background cross-checking, while keeping all
experimental results and analysis fully authored and validated by me.\footnote{ChatGPT (OpenAI) was used for
conceptual clarification. No experimental results or analysis were generated by the tool.}\footnote{Perplexity AI
was used for exploratory questioning and cross-checking background information. All formulas, performance metrics,
and graph generation code were written and validated by me.}
