...
#define LOOPTAG 11      //tag width in the loop predictor
...
#define PERCWIDTH 7     //Statistical corrector  counter width 5 -> 6 : 0.6 %
...
#define NHIST 48        // twice the number of different histories

#define NBANKLOW 15     // number of banks in the shared bank-interleaved for the low history lengths
#define NBANKHIGH 33        // number of banks in the shared bank-interleaved for the  history lengths

int SizeTable[NHIST + 1];


#define BORN 11         // below BORN in the table for low history lengths, >= BORN in the table for high history lengths,

// we use 2-way associativity for the medium history lengths
#define BORNINFASSOC 8      //2 -way assoc for those banks 0.4 %
#define BORNSUPASSOC 17

/*in practice 2 bits or 3 bits par branch: around 1200 cond. branchs*/

#define MINHIST 9       //not optimized so far
#define MAXHIST 4908
...
#define PHISTWIDTH 25       // width of the path history used in TAGE
#define UWIDTH 2        // u counter width on TAGE (2 bits not worth the effort for a 512 Kbits predictor 0.2 %)
...
// LOOPPREDICTOR state
        ...
        PredictionResult predict (uint64_t seq_no, uint8_t piece, UINT64 PC)
        {
            // checkpoint current hist
            pred_time_histories.emplace(get_unique_inst_id(seq_no, piece), active_hist);
            return predict_using_given_hist(seq_no, piece, PC, active_hist, true/*pred_time_predict*/);
        }

        PredictionResult predict_using_given_hist (uint64_t seq_no, uint8_t piece, UINT64 PC, const cbp_hist_t& hist_to_use, const bool pred_time_predict)
        {
            ...
            assert(!((LowConf && MedConf) || (MedConf && HighConf) || (LowConf && HighConf)));

            PredictionResult result;
            result.taken = pred_taken;
            if (HighConf) {
                result.confidence = 2;  // High confidence
            } else if (MedConf) {
                result.confidence = 1;  // Medium confidence
            } else if (LowConf) {
                result.confidence = 0;  // Low confidence
            } else {
                result.confidence = 0; // Not sure why this happen
            }
        ...
        void update (uint64_t seq_no, uint8_t piece, UINT64 PC, bool resolveDir, bool predDir, UINT64 nextPC)
        {
            ...
            PredictionResult pred_taken = predict_using_given_hist(seq_no, piece, PC, pred_time_history, false/*pred_time_predict*/);
            ...
            update(PC, resolveDir, pred_taken.taken, nextPC, pred_time_history);
