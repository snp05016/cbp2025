...
#define BORNTICK 1024 // Tuning has no effect
...
int tage_sc_l_size ()
{
    ...
    if (INB <= 2) {
        inter += INB * (1 << (LOGINB - 1)) * PERCWIDTH;
    } else {
        inter += (INB - 2) * (1 << (LOGINB)) * (PERCWIDTH)
                 + (1 << (LOGINB - 1)) * (2 * PERCWIDTH);
    }
    ...
    if (IMNB <= 2) {
        ...
        inter += (IMNB - 2) * (1 << (LOGIMNB)) * (PERCWIDTH)
                 + (1 << (LOGIMNB - 1)) * (2 * PERCWIDTH);
    }
...
#endif
#ifdef PRINTSIZE
    fprintf (stderr, " (TOTAL %d bits, %d Kbits, %d KBytes) ", STORAGESIZE, (STORAGESIZE+1023) / 1024, (STORAGESIZE+8191) / 8192);
    fprintf (stdout, " (TOTAL %d bits, %d Kbits, %d KBytes) ", STORAGESIZE, (STORAGESIZE+1023) / 1024, (STORAGESIZE+8191) / 8192);
#endif
...
// LOOPPREDICTOR state
        ...
        void update (uint64_t seq_no, uint8_t piece, UINT64 PC, bool resolveDir, UINT64 nextPC, bool filter=false)
        {
