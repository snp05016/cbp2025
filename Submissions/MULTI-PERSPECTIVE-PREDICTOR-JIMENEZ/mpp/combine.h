...
class combine_update : public branch_update {
public:
	// the branch address
	unsigned int pc;
	...
	branch_update *tage_sc_l_u, *mpp_u;
...
class combine : public branch_predictor {
public:
...
#define N_MISS		(1<<MAX_MISS_INDEX_BITS)
	...
	branch_predictor *tage_sc_l_p, *mpp_p;
	...
	void attach (branch_predictor *_tage_sc_l_p, branch_predictor *_mpp_p) {
	...
	bool vote (branch_update *tage_sc_l_u, branch_update *mpp_u) {
	...
	void monitor (branch_update *tage_sc_l_u, branch_update *mpp_u, bool taken) {
		int tage_pred = tage_sc_l_u->prediction();
		int mpp_pred = mpp_u->prediction();
	...
	branch_update *lookup (unsigned int pc, uint64_t dynamic_id) {
		...
		u.mpp_u = mpp_p->lookup (pc, dynamic_id);
	...
	void spec_update (branch_update *p, uint64_t target, bool taken, bool pred, int type, bool, uint64_t dynamic_id) {
	...
	void update (branch_update *p, unsigned int target, bool taken, int type, uint64_t dynamic_id) {
