 ...
 * from O-GEHL.
 * 
 * MPP uses a transfer function to improve accuracy by boosting values of
 ...
 * 
 * MPP also speculatively updates the history tables and training threshold
 ...
 * branches in flight and puts MPP into a non-speculative update mode until
...
// branches in flight. MPP throttles speculative update when it's true

extern int speculatively_update_tables;
...
// MPP histories all in one place
...
// names of features for parsing and printing later (not used for CBP 2025)

#ifndef _MAIN_CC
const char *type_names[] = {
"nothing", "ACYCLIC", "MODHIST", "BIAS", "RECENCY", "IMLI", "PATH", "LOCAL", "MODPATH", "GHISTPATH", "GHISTMODPATH", "BLURRYPATH", "RECENCYPOS", "BACKPATH", "BACKGHISTPATH", "TAGE", 
};
#else
...
public:
	...
	mpp (
		...
		// we will count the number of bits MPP uses
	...
	uint64_t get_hash (history_spec *s, unsigned int pc, unsigned int pc2, int t) {
		uint64_t x = 0;
		extern unsigned int global_tage_pred, global_tage_conf;
		switch (s->type) {
		case BACKGHISTPATH:
	...
	void compute_output (void) {
		...
		for (int i=0; i<num_tables; i++) {
			...
			uint64_t h = get_hash (&myspecs[i], u.pc, u.pc2, i);
	...
	branch_update *lookup (unsigned int pc, uint64_t dynamic_id) {
	...
	void spec_update (branch_update *p, uint64_t target, bool taken, bool pred, int type, bool filtered, uint64_t id) {
...
#define RECORD_FILTERED_RECENCY	128
	...
	void update (branch_update *p, unsigned int target, bool taken, int type, bool do_train, bool filtered, uint64_t dynamic_id) {
		u = * (mpp_update *) p;
