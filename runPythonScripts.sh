...
if [ -f "${RESULTS_CSV}" ]; then
    ...
    echo -e "${RED}✗${NC} results.csv not found in root, results/, or ${SCRIPT_DIR}/."
...
cd ${SCRIPT_DIR}
python3 baselineDifficulty.py > ${PROJECT_ROOT}/${REPORTS_DIR}/06_difficulty_analysis/intrinsic_hardness/baseline_difficulty_report.txt 2>&1
cd ${PROJECT_ROOT}
...
cd ${SCRIPT_DIR}
python3 compareMPKI_vs50PercMPKI.py > ${PROJECT_ROOT}/${REPORTS_DIR}/03_phase_behavior/phase_behavior_report.txt 2>&1
cd ${PROJECT_ROOT}
...
cd ${SCRIPT_DIR}
python3 graphComparator.py > ${PROJECT_ROOT}/${REPORTS_DIR}/01_misprediction_analysis/mpki_comparison_report.txt 2>&1
cd ${PROJECT_ROOT}
...
cd ${SCRIPT_DIR}
python3 compareMetrics.py > ${PROJECT_ROOT}/${REPORTS_DIR}/05_comparative_analysis/divergent_behavior_report.txt 2>&1
cd ${PROJECT_ROOT}
...
cd ${SCRIPT_DIR}
python3 generate_graphs.py 2>&1 | tee ${PROJECT_ROOT}/${REPORTS_DIR}/04_performance_metrics/graph_generation.log
cd ${PROJECT_ROOT}
...
cd ${SCRIPT_DIR}
python3 rangeOfBranchesAcrossBenchmarks.py > ${PROJECT_ROOT}/${REPORTS_DIR}/02_branch_density/branch_density_report.txt 2>&1
cd ${PROJECT_ROOT}
